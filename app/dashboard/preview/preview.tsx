...
import { categoryAgents } from "./agents/data";
...
const findAgentById = (id: string) => {
  for (const category in categoryAgents) {
    const agent = categoryAgents[category].find((agent) => agent.id === id);
...
const ContentCard = ({
  ...
  const agent = findAgentById(result.id);
    ...
    <Card className="h-full overflow-hidden hover:shadow-lg transition-shadow duration-300">
      <CardContent className="p-4">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold">
              {agent ? agent.name : `Response ${result.id}`}
            </h3>
          </div>
        ...
        <ScrollArea
          ...
          } transition-all duration-300 rounded-md bg-gray-50 p-4`}
        >
          {hasError ? (
...
const ContentComparison = () => {
  ...
  const gridClassName = `grid gap-4 p-4 ${
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center">
        ...
        <p className="text-gray-500 mt-2">
          Try refreshing the page or checking your data source.
        </p>
      </div>
    );
  }
